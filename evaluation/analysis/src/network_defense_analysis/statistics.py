...
def _comparison_groups(identities: dict[str, tuple[str, int, int]], comparison: dict) -> tuple[list[str], list[str]]:
    if not isinstance(comparison, dict) or not {"strategy", "baseline", "budget"} <= set(comparison):
    ...
    left = [pid for pid, identity in identities.items() if identity[:2] == (tested, budget)]
    right = [pid for pid, identity in identities.items() if identity[:2] == (baseline, budget)]
